...
export class SaleService {
  ...
  async create(companyId: string, sellerId: string, createSaleDto: CreateSaleDto) {
    try {
      ...
      const invalidMethods = createSaleDto.paymentMethods.filter(
        method => !validPaymentMethods.includes(method as PaymentMethod)
      );

      if (invalidMethods.length > 0) {
        throw new BadRequestException(`Métodos de pagamento inválidos: ${invalidMethods.join(', ')}`);
      }

      // Check if installment payment requires client name
      if (createSaleDto.paymentMethods.includes(PaymentMethod.INSTALLMENT) && !createSaleDto.clientName) {
        throw new BadRequestException('Nome do cliente é obrigatório para vendas a prazo');
      }

      // Calculate change
      const totalPaid = createSaleDto.totalPaid || 0;
      ...
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const sale = await tx.sale.create({
          data: {
            ...
            paymentMethod: createSaleDto.paymentMethods,
            change,
            isInstallment: createSaleDto.paymentMethods.includes(PaymentMethod.INSTALLMENT),
            companyId,
            sellerId,
          },
        });
      ...
      // Print receipt
      try {
        await this.printerService.printReceipt(completeSale);
      } catch (printError) {
        this.logger.warn('Failed to print receipt:', printError);
      }
