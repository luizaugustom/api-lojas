...
export class SanitizeUpdateDataInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (request.params && request.params.id) {
      ...
      // Regex para validar CUID (Collision-resistant Unique Identifier)
      const cuidRegex = /^c[a-z0-9]{24,25}$/i;
      
      if (!uuidRegex.test(id) && !cuidRegex.test(id)) {
        throw new BadRequestException(`ID inválido: ${id}. Esperado formato UUID ou CUID válido (ex: 123e4567-e89b-12d3-a456-426614174000 ou cmgtzxtsd0001goxmzmitp7s4)`);
      }
    }
