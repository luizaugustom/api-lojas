...
describe('CompanyService - Fiscal Configuration', () => {
  ...
  describe('updateFiscalConfig', () => {
    it('deve atualizar configurações fiscais com sucesso', async () => {
      ...
      const updateDto: UpdateFiscalConfigDto = {
        taxRegime: 'SIMPLES_NACIONAL' as any,
        cnae: '4761001',
        municipioIbge: '4205407',
        nfceSerie: '1',
    ...
    it('deve lançar erro se empresa não encontrada', async () => {
      ...
      await expect(
        service.updateFiscalConfig('invalid-id', {})
    ...
    it('deve atualizar apenas campos fornecidos', async () => {
      const updateDto: UpdateFiscalConfigDto = {
        municipioIbge: '4209102', // Apenas município
      };
  ...
  describe('getFiscalConfig', () => {
    ...
    it('deve lançar erro se empresa não encontrada', async () => {
      ...
      await expect(
        service.getFiscalConfig('invalid-id')
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('uploadCertificateToFocusNfe', () => {
    ...
    beforeEach(() => {
      // Mock axios para não fazer chamadas reais
      jest.mock('axios');
    });

    it('deve validar se arquivo foi enviado', async () => {
      await expect(
        service.uploadCertificateToFocusNfe('company-123', null as any)
      ).rejects.toThrow('Arquivo de certificado é obrigatório');
    });
    ...
    it('deve validar extensão do arquivo', async () => {
      ...
      await expect(
        service.uploadCertificateToFocusNfe('company-123', invalidFile)
      ).rejects.toThrow('Arquivo deve ser .pfx ou .p12');
    });
    ...
    it('deve validar se API Key está configurada', async () => {
      ...
      await expect(
        service.uploadCertificateToFocusNfe('company-123', mockFile)
      ).rejects.toThrow('API Key do Focus NFe não configurada');
    });
    ...
    it('deve validar se senha do certificado está configurada', async () => {
      ...
      await expect(
        service.uploadCertificateToFocusNfe('company-123', mockFile)
      ).rejects.toThrow('Configure a senha do certificado antes');
    });
    ...
    it('deve aceitar arquivo com extensão .p12', () => {
      ...
      // Validar apenas a extensão (não executar upload real)
  ...
  describe('validações de dados fiscais', () => {
    it('deve validar código IBGE com 7 dígitos', () => {
      ...
      // Teste através do DTO (validação em runtime)
